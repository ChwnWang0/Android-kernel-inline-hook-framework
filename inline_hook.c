 ...
 * @LastEditTime: 2026-04-28 23:36:57
 * @FilePath: /inlinehooktest/inline_hook.c
...
static int build_relo_insts(struct sym_hook *hook)
{
    ...
    pr_info("inline_hook: built %d relocated instructions\n", hook->relo_insts_num);
    return 0;
}
...
static void *follow_trampoline(void *target)
{
    ...
    if (first_inst == 0xd503245f && (second_inst & 0xfc000000) == 0x14000000) {
        ...
        pr_info("inline_hook: followed trampoline to %p\n", target);
    }
    return target;
}
...
static __nocfi bool check_function_length_enough(void *target)
{
    ...
    if (symbolsize < HIJACK_SIZE + offset) {
        pr_warn("inline_hook: symbol size %lu offset %lu too short for hijack\n",
            symbolsize, offset);
        return false;
    }
    ...
    for_each_process_thread(g, t) {
        ...
        for (i = 0; i < nr_entries; i++) {
            if (entries[i] >= addr && entries[i] < addr + HIJACK_SIZE) {
                pr_warn("inline_hook: stack conflict PID %d Comm %.20s [<%lx>]\n",
            ...
            if (hook_func && kallsyms_lookup_size_offset_ptr) {
                ...
                if (kallsyms_lookup_size_offset_ptr(hook_func, &symbolsize, &offset)) {
                    if (entries[i] >= hook_func - offset &&
                        ...
                        pr_warn("inline_hook: hook_func conflict PID %d Comm %.20s\n",
                            t->pid, t->comm);
                        ret = -1;
                    }
                }
...
int inline_hook_init(void)
{
    ...
    if (!stop_machine_ptr) {
        pr_err("inline_hook: stop_machine not found\n");
        return -ENOENT;
    }

    if (!kallsyms_lookup_size_offset_ptr)
        pr_warn("inline_hook: kallsyms_lookup_size_offset not found, skip length check\n");
    if (!stack_trace_save_tsk_ptr)
        pr_warn("inline_hook: stack_trace_save_tsk not found, skip stack safety check\n");

    g_runtime_ready = true;
    pr_info("inline_hook: runtime ready\n");
    return 0;
}
    ...
    void *hook_template_code_space, void *hook_func, char *mod_name)
{
    ...
    if (!check_function_length_enough(target)) {
        pr_err("inline_hook: %lx shorter than hijack_size %d\n",
    ...
    hash_for_each_possible(all_hijack_targets, sa, node, ptr_hash) {
        if (target == sa->target) {
            ...
            pr_err("inline_hook: %lx already prepared\n", (unsigned long)target);
            return -1;
        }
    }
    up_read(&hijack_targets_hashtable_lock);
    ...
    if (!sa || (mod_name && !name)) {
        kfree(sa);
        kfree(name);
        pr_err("inline_hook: no memory for %lx\n", (unsigned long)target);
        return -ENOMEM;
    }
    ...
    if (hook_template_code_space) {
        if (build_relo_insts(sa) < 0) {
            kfree(sa->mod_name);
            kfree(sa);
            pr_err("inline_hook: failed to build relocated instructions for %lx\n",
                (unsigned long)target);
            return -1;
        }
    }
    ...
    hash_for_each_possible_safe(all_hijack_targets, sa, tmp, node, ptr_hash) {
        if (sa->target == target) {
            if (!sa->enabled) {
                ...
                pr_info("inline_hook: %lx already enabled\n", (unsigned long)target);
                ret = 0;
            }
            goto out;
        }
    }
    pr_err("inline_hook: %lx not prepared\n", (unsigned long)target);
out:
    up_write(&hijack_targets_hashtable_lock);
    return ret;
}
